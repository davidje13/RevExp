...
function resolveForward(inputCharsString, beginNodes, endNode, checkLoops) {
	...
	return {
		beginState,
		endState: positions[length].states.get(endNode),
	};
}

function resolveSolutionWithOverlap(inputCharsString, beginState, endState) {
	const length = inputCharsString.length;

	const active = [endState];
	while (active.length) {
		const a = active.pop();
	...
	return beginState.solution;
}

function resolveSolutionNoOverlap(inputCharsString, beginState, endState) {
	...
	return solution.map((p, i) => (inputCharsString[i].intersect(p)));
}
...
export default class RevExp {
	...
	reverse(value, unknown = null) {
		...
		const { beginState, endState } = resolveForward(inputCharsString, this.beginNodes, this.endNode, checkLoops);
		if (!endState) {
			return null;
		}

		const solution = (this.graphContainsOverwrites
			? resolveSolutionWithOverlap(inputCharsString, beginState, endState)
			: resolveSolutionNoOverlap(inputCharsString, beginState, endState)
		);
		return solution ? CharacterClassString(solution) : null;
	}
